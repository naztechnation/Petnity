...
import '../../../../model/user_models/id_types.dart';
...
class KycServiceScreenTen extends StatelessWidget {
  const KycServiceScreenTen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return BlocProvider<AccountCubit>(
      ...
      child: KycServiceScreen(),
    );
  }
}
class KycServiceScreen extends StatefulWidget {
    KycServiceScreen({super.key});

  @override
  State<KycServiceScreen> createState() => _KycServiceScreenState();
}

class _KycServiceScreenState extends State<KycServiceScreen> {
  ...
  getServicesTypes() async {
    ...
    //   isLoading = true;
    // });
    await _accountCubit.getIdTypes();
    // setState(() {
    //   isLoading = false;
    // });
  }
    ...
    return Scaffold(
      body: BlocConsumer<AccountCubit, AccountStates>(
        listener: (context, state) {
          if (state is IdTypeUpdated) {
           idTypes = state.id.data?.idTypes ?? [];


             for (var ids in idTypes ?? []){
           options.add(ids.name);
           iDs.add(ids.sId);
        ...
        builder: (context, state) => (state is AccountProcessing)
            ? Container(
                ...
                child: Center(child: LoadingPage())): SingleChildScrollView(
          child: Container(
             height: screenSize(context).height,
        width: screenSize(context).width,
                  ...
                  decoration: BoxDecoration(
                    ...
                    onChanged: (value) {
                       selectedOption = value!;
                        serviceProvider.setPhotoId(iDs[options.indexOf(value)]);
                      },
